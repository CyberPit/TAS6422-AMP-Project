...
static unsigned char led_dot[3] = {0xff, 0xff, 0xff};
...
# 302 "mcc_generated_files/tmr1.c"
signed char readRE(void){
...
const signed char RE_states[] = {0,-1, 0, 5, 1, 0, 5, 0, 0, 5, 5, 0, 5, 0, 0, 0};
 static signed char direction;
  static unsigned char RE_old = 0;
  unsigned char RE_now;
  signed char offset;
  RE_now = PORTAbits.RA5 * 2 + PORTAbits.RA4;
  RE_old <<= 2;
  RE_old |= ( RE_now & 0x03 );
  if ((offset = RE_states[( (RE_old & 0x0F))]) == 5)
    return (1 * direction * offset);
  direction = offset;
  return(offset);
}
...
signed char read_clickRE(void){
  static unsigned char index, interval;
     ...
     current = PORTAbits.RA5;
   if((current != index) && (current== 0))
        {
        index =current;
        current = PORTAbits.RA4 ? 1 : -1;
        if (interval <5)
           current *=2;
        if (interval <12)
           current *=2;
